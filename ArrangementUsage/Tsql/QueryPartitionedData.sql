SELECT x.*,'ALTER TABLE ' + x.TableName + ' SWITCH  PARTITION ' +  
CAST($partition.pf_Daily(cast(x.UpperBoundary as date)) As VARCHAR)  +' TO staging_' + x.TableName + '_OUT PARTITION ' + CAST($partition.pf_Daily(cast(x.UpperBoundary as date)) As VARCHAR) AS 'FixQuery'
			FROM (
SELECT
...
WHERE
	...
	--AND OBJECT_NAME(p.OBJECT_ID) IN ('staging_Subscriber_BTS_FIRST_TOPUP_20211202') 
	AND CASE WHEN fg.name IS NULL THEN ds.name ELSE fg.name END = 'Data' AND CASE WHEN p.index_id IN (0,1) THEN p.ROW_COUNT ELSE 0 END > 0
	) x
				--WHERE cast(x.UpperBoundary as date) >  '2022-02-12' --AND x.[RowCount] > 0